...
describe("bitcoinutil", function () {
  ...
  it("sign", function(){
    ...
    var signedTx   = bitcoinutil.sign({ tx: tx, privateKey: privateKey, redeem: redeem })
    console.log('sign transaction.', 'signedTx', signedTx)
    expect(signedTx).to.be.eql(fixtures.sign.signedtx)
  })
})

/*
 var keys = bitcoinutil.makeRandom()
 console.log(keys)
 keys = bitcoinutil.addressFromPrivateKey(keys.privateKey)
 console.log(keys)*/
